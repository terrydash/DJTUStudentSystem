...
namespace DJTUStudentSystem.Config
{
    ...
    {
        public static bool AllowChooseChourse = true;//是否允许选课
        public static DateTime StartChooseTime { get; set; }//开始选课时间        
        public static DateTime EndChooseTime { get; set; }//结束选课时间
        public static int AllowConfictWeeks = 3;//选课时，所选的课程与学生课表存在冲突时，允许冲突几周
        public static bool isReadFromDB = false;//是否忽略缓存，从数据库直接读取数据；
        private static List<GradeCanChooseCourseModel> _GradeCanChooseCourseModelList = new List<GradeCanChooseCourseModel>();
        public static List<GradeCanChooseCourseModel> GradeCanChooseCourse//哪个年纪可以选课
        {
            get
            {if (_GradeCanChooseCourseModelList.Count<=0)
                { 
            ...
            set
            {
                _GradeCanChooseCourseModelList = value;
            }
